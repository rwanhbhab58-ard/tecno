...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    let count = 0;
    const effectiveColors = Array.isArray(colors) && colors.length > 0
      ? colors
      : [color0, color1, color2].filter(Boolean);

    if (effectiveColors.length > 0) {
      const gl = renderer.gl;
      const capped = effectiveColors.slice(0, 64);
      count = capped.length;
  ...
  }, [intensity, speed, animationType, colors, color0, color1, color2, distort, offset, rayCount, lightMode]);
