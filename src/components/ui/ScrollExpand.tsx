...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  const isReducedMotion =
    ...
    (typeof window !== 'undefined'
      ? new URLSearchParams(window.location.search).get('reduced-motion') === 'true'
      : false);
  ...
  const applyProgress = useCallback((p: number) => {
    ...
    if (titleRef.current) {
      const out = smoothstep(0.12, 0.65, p);
      titleRef.current.style.opacity = `${1 - out}`;
      titleRef.current.style.transform = `translate3d(0, ${-32 * out}px, 0) scale(${1 + 0.05 * out})`;
      titleRef.current.style.pointerEvents = out > 0.8 ? 'none' : 'auto';
    }
    ...
    if (overlayRef.current) {
      const inn = smoothstep(0.68, 0.98, p);
  ...
  return (
    <div
      ref={rootRef}
      className={`scroll-expand ${useWindowScroll ? '' : 'scroll-expand--scroller'} ${isReducedMotion ? 'scroll-expand--reduced-motion' : ''} ${className}`.trim()}
      style={style}
      {...rest}
    >
